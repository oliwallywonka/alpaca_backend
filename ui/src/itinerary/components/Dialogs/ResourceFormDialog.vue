...
import ResourceProviderPicker from '@/itinerary/components/ResourceProviderPicker/ResourceProviderPicker.vue'
import { Badge } from '@/core/components/ui/badge'
import { Button } from '@/core/components/ui/button'

const isOpen = defineModel<boolean>({ default: false })
const props = defineProps<{
  itineraryResource: Partial<ItineraryResource>
}>()
const currentResource = ref<ResourceProvider | undefined>()
const create = ItineraryResourceService.useCreate()
...
const handleSubmit = async () => {
  ...
  try {
    await create.mutateAsync({
      ...props.itineraryResource,
      resourceProvider: currentResource.value.id,
      minPersons: currentResource.value.refPrices[0].minPersons,
      maxPersons: currentResource.value.refPrices[0].maxPersons,
    ...
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>
          ...
          section" {{ props.itineraryResource }}
        </DialogDescription>
      </DialogHeader>
      ...
      <ResourceProviderPicker @select:resourceProvider="currentResource = $event" />
