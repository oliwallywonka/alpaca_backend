...
const { data } = TourVariantService.useGetOne(toRef(props.variantID as string), startFetching)
const update = TourVariantService.useUpdate()
const create = TourVariantService.useCreate()
...
const onSubmit = handleSubmit(async (values) => {
  try {
    ...
    if (!props.variantID) {
      await create.mutateAsync({ ...values, tour: props.tourID, isActive: true })
